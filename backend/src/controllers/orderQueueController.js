...
const getOrders = async (req, res) => {
  try {
    ...
    for (const order of orders) {
      const items = await pool.request().input("order_id", order.id).query(`
...
const getOrderById = async (req, res) => {
  try {
    ...
    const items = await pool.request().input("id", req.params.id).query(`
        SELECT oi.quantity, oi.unit_price, m.name AS item_name, m.item_type
        FROM OrderItems oi JOIN MenuItems m ON oi.menu_item_id = m.id
    ...
    if (order.order_type === "GROUP") {
      const members = await pool.request().input("id", req.params.id).query(`
...
const updateOrderStatus = async (req, res) => {
  try {
    ...
    if (!transitions[cur]?.includes(status)) {
      return res
        .status(400)
        .json({ error: `Cannot move from ${cur} to ${status}.` });
    }
